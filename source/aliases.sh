# ------------------------------------------------------------------------------
# Shell aliases.
# ------------------------------------------------------------------------------

# Navigation.
alias ..="cd .."
alias ...="cd ../.."
alias ....="cd ../../.."
alias -- -="cd -"

# Make.
alias mc="make check"
alias mr="make release"
alias md="make debug"
alias mds="make debug-sanitized"

alias mcd="make check-debug"
alias mcds="make check-debug-sanitized"
alias mcr="make check-release"

alias mt="make test"
alias mtv="make test-verbose"
alias mti="make test-integration"
alias mtiv="make test-integration-verbose"

# Applications.
alias vv="vim"
alias arkd="python3 $HOME/dev/src/ark/ark"
alias hb="hexbomb"
alias irn="ironclad"
alias int="intspector"
alias vimvd="$HOME/dev/src/vimv/target/debug/vimv"
alias ca="code -a ."
alias tc="termcalc"

# Commands.
alias dir="open ."
alias dush="du -sh"
alias hosts="sudo vim /etc/hosts"

# Pyro.
alias pd="$HOME/dev/code/pyro/build/debug/pyro"
alias pdt="$HOME/dev/code/pyro/build/debug/pyro test"
alias pdtv="$HOME/dev/code/pyro/build/debug/pyro test -v"
alias pdta="$HOME/dev/code/pyro/build/debug/pyro test $HOME/dev/code/pyro/tests/*.pyro"
alias pdtav="$HOME/dev/code/pyro/build/debug/pyro test -v $HOME/dev/code/pyro/tests/*.pyro"
alias pr="$HOME/dev/code/pyro/build/release/pyro"
alias prt="$HOME/dev/code/pyro/build/release/pyro test"
alias prtv="$HOME/dev/code/pyro/build/release/pyro test -v"
alias prta="$HOME/dev/code/pyro/build/release/pyro test $HOME/dev/code/pyro/tests/*.pyro"
alias prtav="$HOME/dev/code/pyro/build/release/pyro test -v $HOME/dev/code/pyro/tests/*.pyro"

# History: 'hn' loads new history entries from other terminal windows.
alias hh="history 25"
alias hhh="history 45"
alias hn="history -a; history -n"
alias hl="history -a; history -n"

# Git.
alias ga="git add"
alias gaa="git add --all"

alias gd="git diff"
alias gds="git diff --staged"

alias gs="git status --short --branch"
alias gb="git branch"

alias gc="git commit"
alias gcm="git commit -m"
alias gcac="git commit -am Cleanup"
alias gcad="git commit -am 'Update documentation'"
alias gcah="git commit -am Housekeeping"
alias gcam="git commit -am"
alias gcar="git commit -am 'Update readme'"
alias gcas="git commit -am Snapshot"
alias gcat="git commit -am 'Update tests'"
alias gcaw="git commit -am WIP"

alias gl="git log --oneline --graph -25"
alias gll="git log --oneline --graph -45"

alias gpt="git push --tags"

# Listings.
alias ls="ls -F"
alias ls1="ls -F1"
alias la="ls -FA"
alias la1="ls -FA1"
alias ll="ls -Flh"
alias lla="ls -FlhA"

# Tree.
alias tt="tree -F"
alias t2="tree -F -L 2"
alias t3="tree -F -L 3"
alias t4="tree -F -L 4"
alias t5="tree -F -L 5"
alias tta="tree -F -a"
alias tts="tree -F -h"
alias ttas="tree -F -a -h"
alias ttsa="tree -F -a -h"

# Sudo: preserve the existing shell environment; same as -E.
alias sudo="sudo --preserve-env"

# Browsers.
alias chrome="open -a 'Google Chrome'"
alias safari="open -a Safari"

# Miscellanea.
alias mkdir="mkdir -p"
alias serve="python3 -m http.server 8080"

# Calender: brew install util-linux
alias cal="cal -3 --monday"

# Go.
alias gt="go test ./..."
alias gti="go test -tags integration ./..."
alias goclean="go clean --cache --testcache --modcache --fuzzcache"
